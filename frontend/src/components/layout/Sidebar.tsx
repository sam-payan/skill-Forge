...
import { LayoutDashboard, Map, ClipboardCheck, MessageSquare, User, Settings, ChevronLeft, Zap, Trophy, Sparkles } from "lucide-react";
...
const Sidebar = forwardRef<HTMLDivElement, SidebarProps>(({ collapsed, mobileMenuOpen, onNavClick, onClose }, ref) => {
  ...
  return (
    <aside
      ref={ref}
      className={cn(
        "fixed inset-y-0 left-0 z-40 bg-sidebar border-r border-sidebar-border flex flex-col transition-transform duration-300 lg:translate-x-0",
        collapsed ? "w-20" : "w-64",
    ...
    >
      ...
      <div className="flex items-center gap-3 px-6 h-16 border-b border-sidebar-border">
        ...
        {!collapsed && <span className="font-bold text-lg text-sidebar-accent-foreground">SkillForge</span>}
      </div>
      ...
      {!collapsed && (
        <>
          ...
          <div className="mx-3 mb-4 p-4 rounded-xl bg-gradient-to-br from-primary/20 to-accent/20 border border-primary/20">
            ...
            <p className="text-xs text-sidebar-foreground mb-3">Get personalized guidance for your learning journey</p>
            {mentorLink && (
              <Link to={mentorLink}>
                <Button size="sm" variant="accent" className="w-full text-xs">Start Chat</Button>
              </Link>
            )}
          </div>
        </>
